...
import com.io.ab.music.ui.components.LandscapeSidebar
import com.io.ab.music.ui.components.LandscapeQueuePanel
import com.io.ab.music.ui.components.LandscapeBottomBar
...
fun ABMusicNavGraph(
    ...
    // Landscape PRD (structure-only): main tab shell rearranges into
    // Sidebar | Main Content | Queue Panel + a full-width bottom mini
    // player instead of the portrait top tab bar + floating mini player.
    // Same screens, same ViewModels, same nav destinations either way.
    val isLandscape = LocalConfiguration.current.orientation ==
        android.content.res.Configuration.ORIENTATION_LANDSCAPE

    val currentSongId by playerViewModel.currentSongId.collectAsState()
    val playerState    by playerViewModel.playerState.collectAsState()
    val currentSongIsFavorite by playerViewModel.currentSongIsFavorite.collectAsState()
    ...
    Scaffold(
        ...
        Box(modifier = Modifier.padding(innerPadding).fillMaxSize()) {
            ...
            if (!isFullScreen && isLandscape) {
                // ── Landscape shell (PRD: Sidebar | Main Content | Queue Panel, full-width bottom bar) ──
                Column(modifier = Modifier.fillMaxSize()) {
                    Row(modifier = Modifier.weight(1f).fillMaxWidth()) {
                        ...
                        LandscapeQueuePanel(
                            playerState      = playerState,
                            onQueueItemClick = { idx -> playerViewModel.playQueue(playerState.queue, idx) },
                            onPlayerClick    = { navController.navigate(Screen.Player.route) }
                        )
                    }
                    AnimatedVisibility(
                        visible = showMiniPlayer && settingsUiState.landscapeMiniPlayerEnabled,
                        enter   = fadeIn(tween(220)) + slideInVertically(tween(220)) { it },
                        exit    = fadeOut(tween(180)) + slideOutVertically(tween(180)) { it }
                    ) {
                        LandscapeBottomBar(
                            playerState      = playerState,
                            isFavorite       = currentSongIsFavorite,
                            onPlayerClick    = { navController.navigate(Screen.Player.route) },
                            ...
                            onToggleFavorite = { currentSongId?.let { playerViewModel.toggleFavorite(it) } },
                            onQueueClick     = { navController.navigate(Screen.Queue.route) }
                        )
                    }
                }
            } else if (!isFullScreen) {
                // ── Portrait shell (unchanged): swipeable pager + floating mini player ──
                TabPagerContent(
                    pagerState      = pagerState,
                    navController   = navController,
                    playerViewModel = playerViewModel,
...
// ── Shared tab content (Home/Video/Explore/Playlists/Favorites/Settings) ──────
// Used as-is by both the portrait pager (full screen) and the landscape shell
// (inside the Main Content column) — identical swipe behaviour and screens in
// both orientations, only the surrounding shell layout differs.
@OptIn(androidx.compose.foundation.ExperimentalFoundationApi::class)
@Composable
