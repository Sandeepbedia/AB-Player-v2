...
data class SettingsUiState(
    ...
    val landscapeMiniPlayerEnabled : Boolean = false,
...
class SettingsViewModel @Inject constructor(
    ...
    private fun loadPersistedPreferences() {
        ...
        viewModelScope.launch {
            prefs.landscapeMiniPlayerEnabled.collect { enabled ->
                _uiState.update { it.copy(landscapeMiniPlayerEnabled = enabled) }
            }
        }
        // Accent color — same one-shot pattern as wallpaperBlur/wallpaperDim above:
        // the picker dialog drives uiState.accentColor live while dragging via
        // setAccentColorPreview(), so we only need the persisted value ONCE on
        // startup, never re-collected afterwards (or a delayed DataStore emit
        // would snap the swatch back mid-drag).
        viewModelScope.launch {
            _uiState.update { it.copy(accentColor = prefs.accentColor.first()) }
        }
    }
    ...
    // ── Landscape mode bottom mini player toggle ──────────────────────────────
    fun setLandscapeMiniPlayerEnabled(enabled: Boolean) {
        viewModelScope.launch { prefs.setLandscapeMiniPlayerEnabled(enabled) }
        _uiState.update { it.copy(landscapeMiniPlayerEnabled = enabled) }
    }
    ...
    // ── Accent Color (RGB picker) ───────────────────────────────────────────
    /** Live swatch update while dragging inside the picker — no disk write. */
    fun setAccentColorPreview(hex: String) {
        _uiState.update { it.copy(accentColor = hex) }
    }

    /** Persist the final color when the user taps Done. */
    fun setAccentColor(hex: String) {
        viewModelScope.launch { prefs.setAccentColor(hex) }
        _uiState.update { it.copy(accentColor = hex) }
    }
    ...
    private val CURRENT_VERSION_CODE = 427   // Must match versionCode in app/build.gradle
